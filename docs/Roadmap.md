...
* **Max impact cap**: the symmetric guard `SemverPolicy.MaxImpact` (+ `--allow-major`) blocks fragments/releases that would force a Major bump the team does not want. Enforced at `new` and `release`. See [SemVer Rules](SemVer%20Rules.md).
