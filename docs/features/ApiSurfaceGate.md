...
The API Surface Gate is a **floor** (fragments must not be lower than the real
impact). ChangeSharp also provides the symmetric **cap** so a fragment can never
silently force a Major bump the team does not want:

* `SemverPolicy.MaxImpact` in `changesharp.json` (default `major` = cap off).
* Enforced at `changesharp new` (creation) and `changesharp release` (production),
  both requiring the explicit `--allow-major` flag to proceed.

See [SemVer Rules](../SemVer%20Rules.md) for details, and
`samples/maximpact-gate/` for a runnable demo.
