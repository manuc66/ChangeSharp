...
### Fixed
- Push NuGet packages before creating the git release commit/tag so a failed publish does not leave a release without packages
...
### Fixed
- Add explicit permissions block to the build job (CodeQL missing-workflow-permissions)
...
### Fixed
- Fix SonarCloud quality gate findings: regex timeouts, absolute git path, job-scoped workflow permissions, locked docs dependencies, pinned Codecov action
